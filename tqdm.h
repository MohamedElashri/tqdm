...
private:
    ...
    // Rate calculation with lock-free ring buffer
...
public:
    ...
    void set_total(std::size_t total) noexcept {
    ...
    double get_rate() const noexcept {
        ...
        // Check cache first
        {
            ...
            if (std::chrono::duration_cast<std::chrono::milliseconds>(
                    now - cache_.last_update).count() < 100) {
                return cache_.rate;
            }
        }
        ...
        std::size_t entries_to_check = std::min(HISTORY_SIZE, current_idx);
        ...
        if (newest_time > oldest_time && newest_idx != oldest_idx) {
            auto progress_diff = history_[newest_idx].progress.load() - 
                                history_[oldest_idx].progress.load();
            auto time_diff = newest_time - oldest_time;
            
            if (time_diff > 0) {
                rate = 1e6 * static_cast<double>(progress_diff) / 
        ...
        // Update cache
        {
            ...
            cache_.elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                now - start_time_
            );
        }
...
public:
    ...
    void render(const progress_tracker<>& tracker) override {
        ...
        if (show_percentage_) {
            oss << std::setw(3) << std::fixed << std::setprecision(0) 
                << percentage << "% ";
        }
        ...
        // Track width for clearing
        auto current_width = static_cast<int>(output.length());
        auto last = last_width_.exchange(current_width);
        
        // Add spaces to clear previous line if needed
        if (last > current_width) {
            output.append(last - current_width, ' ');
        }
...
public:
    explicit progress_bar(T total, 
        ...
        : tracker_(static_cast<std::size_t>(total))
        , display_(display ? std::move(display) 
                          : std::unique_ptr<display_policy>(
                              new bar_display<decltype(themes::unicode)>())) {
        // Initial render
        if (is_tty()) {
    ...
    progress_bar& operator+=(std::size_t n) {
        advance(n);
        return *this;
    }
    ...
    std::size_t current() const { return tracker_.current(); }
    std::size_t total() const { return tracker_.total(); }
    double percentage() const { return tracker_.percentage(); }
    double rate() const { return tracker_.get_rate(); }
...
public:
    explicit progress_range(ContainerT& container)
        ...
        , bar_(std::distance(std::begin(container), std::end(container))) {}
    ...
    auto begin() -> progress_iterator<iterator_t, progress_bar<>> {
        return progress_iterator<iterator_t, progress_bar<>>(
            std::begin(*container_), std::end(*container_), &bar_
        );
    }
    
    auto end() -> progress_iterator<iterator_t, progress_bar<>> {
        return progress_iterator<iterator_t, progress_bar<>>(
            std::end(*container_), std::end(*container_), nullptr
        );
    }
    
    auto begin() const -> progress_iterator<const_iterator_t, progress_bar<>> {
        return progress_iterator<const_iterator_t, progress_bar<>>(
            std::begin(*container_), std::end(*container_), &bar_
        );
    }
...
inline auto tqdm(ContainerT& container) -> progress_range<ContainerT> {
    return progress_range<ContainerT>(container);
}

// Overload for custom display
template<typename ContainerT>
inline auto tqdm(ContainerT& container, const std::string& label) 
    -> progress_range<ContainerT> {
...
inline progress_bar<> tqdm_manual(std::size_t total, ThemeT theme) {
    return progress_bar<>(total, 
        std::unique_ptr<display_policy>(new bar_display<ThemeT>(theme)));
}
...
template<typename ContainerT>
inline auto tqdm(ContainerT&& container) {
    using DecayedT = typename std::decay<ContainerT>::type;
    return progress_range<DecayedT>(std::forward<ContainerT>(container));
}
